...
num_epochs = 100 # int(sys.argv[3])
data_dir = '../CropFace/out/' + sys.argv[1]
save_dir = sys.argv[1] + '/'
...
# Plot losses
def plot_loss(d_losses, g_losses, num_epoch, save=False, save_dir=sys.argv[1] + '/', show=False):
    fig, ax = plt.subplots()
    ...
    if save:
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_fn = save_dir + sys.argv[1] + '_losses_epoch_{:d}'.format(num_epoch + 1) + '.png'
        plt.savefig(save_fn)
...
def plot_result(generator, noise, num_epoch, save=False, save_dir=sys.argv[1] + '/', show=False, fig_size=(5, 5)):
    generator.eval()
    ...
    if save:
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_fn = save_dir + sys.argv[1] + '_epoch_{:d}'.format(num_epoch+1) + '.png'
        plt.savefig(save_fn)
...
#if(os.path.isfile(sys.argv[1] + '_generator_param.pkl')):
#	G.load_state_dict(torch.load(sys.argv[1] + '_generator_param.pkl'))
#	print("G loaded successfully")

#if(os.path.isfile(sys.argv[1] + '_discriminator_param.pkl')):
#	D.load_state_dict(torch.load(sys.argv[1] + '_discriminator_param.pkl'))
...
for epoch in range(num_epochs):
    ...
    save_fn1 = save_dir + sys.argv[1] + '_losses_epoch_{:d}'.format(epoch + 1) + '.png'
    loss_plots.append(imageio.imread(save_fn1))

    save_fn2 = save_dir + sys.argv[1] + '_epoch_{:d}'.format(epoch + 1) + '.png'
    gen_image_plots.append(imageio.imread(save_fn2))
	
#torch.save(G.state_dict(), sys.argv[1] + 'generator_param.pkl')
#torch.save(D.state_dict(), sys.argv[1] + 'discriminator_param.pkl')

imageio.mimsave(save_dir + sys.argv[1] + '_losses_epochs_{:d}'.format(num_epochs) + '.gif', loss_plots, fps=5)
imageio.mimsave(save_dir + sys.argv[1] + '_epochs_{:d}'.format(num_epochs) + '.gif', gen_image_plots, fps=5)
...
fname = "./" + sys.argv[1] + "/" + sys.argv[1] + "_epoch_100.png"
